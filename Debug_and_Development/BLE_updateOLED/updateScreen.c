...
// ===================== OLED (Code 2, UART removed) =====================
// ---------------- Pin mapping ----------------
#define OLED_SCL_BIT   BIT5   // P1.5
#define OLED_SDA_BIT   BIT4   // P1.4
#define OLED_RES_BIT   BIT6   // P1.6
...
// -----------------------------------------------------------------------------
// Font tables (kept from your Code 2; unchanged)
// -----------------------------------------------------------------------------
...
void OLED_Init_DCDC(void)
{
    ...
    OLED_WR_Byte(0xA1, OLED_CMD); // seg remap
...
// ===================== PACKET -> OLED STRING HELPERS (from Code 2) =====================
...
{
    // pkt layout (14 bytes):
    // [0]=0xFF, [1]=dir, [2]=dist1, [3]=dist2, [4]=unit, [5..12]=street(8), [13]=special
    uint8_t dir;
    ...
    special = pkt[13];
    (void)special; // placeholder for later
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (accepted_count() != 0) {
            ...
            // Forward raw 14 bytes to PC (kept from Code 1 for debugging)
