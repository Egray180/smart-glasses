...
// ---------------- Pin mapping ----------------
#define OLED_SCL_BIT   BIT7   // P1.7
#define OLED_SDA_BIT   BIT6   // P1.6
#define OLED_RES_BIT   BIT5   // P1.5
...
void OLED_ShowStringCentered(uint8_t y, const uint8_t *chr, uint8_t sizey) {
    ...
    if (sizey == 8) {
        OLED_ShowString((60-len*6)/2, y, chr, sizey);
    }
    else {
        OLED_ShowString((60-len*16)/2, y, chr, sizey);
    }
}
...
void OLED_Init(void)
{
    ...
    //OLED_WR_Byte(0xA1, OLED_CMD); // mirrors left/right
    //OLED_WR_Byte(0xC0, OLED_CMD); // mirrors top/bottom
...
{
    oled_gpio_init();
    delay_s(3); // time to turn on VBAT for testing
    oled_reset_pulse();
...
int main(void) {
    ...
    OLED_ShowStringCentered(0,"Welcome",8);
    OLED_ShowStringCentered(1,"to",8);
    OLED_ShowStringCentered(2,"EYEWHERE",8);
